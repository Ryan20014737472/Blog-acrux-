...
export function ConfirmationDialog({ request, busy, onCancel, onConfirm }: {
  ...
  return (
    <dialog
      ...
      className="fixed inset-0 m-auto max-h-[calc(100dvh-2rem)] w-[calc(100vw-2rem)] max-w-lg overflow-y-auto overscroll-contain rounded-3xl border border-cyan-200/25 bg-[#081a36] p-0 text-white shadow-[0_28px_90px_rgba(0,0,0,0.65)] backdrop:bg-[#010610]/85"
      onCancel={(event) => { if (busy) event.preventDefault(); else onCancel(); }}
    >
      <div className="border-b border-white/10 border-l-2 border-l-acrux-cyan-bright bg-gradient-to-r from-[#0e2c55] to-[#081a36] px-6 py-5 sm:px-8">
        <h2 id={titleId} className="text-balance text-xl font-bold sm:text-2xl">{request.title}</h2>
      </div>
