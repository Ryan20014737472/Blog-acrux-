...
export function SponsorsPreview() {
  ...
  return <div className="glass-panel mt-14 rounded-3xl p-6 sm:p-8">
    ...
    {sponsors.length ? <ul className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{sponsors.map((sponsor) => {
      ...
      const content = <><div className="flex h-28 items-center justify-center rounded-xl border border-white/10 bg-acrux-navy p-4">{logo ? <Image alt={`Logo de ${sponsor.name}`} className="max-h-full max-w-full object-contain" height={96} loading="lazy" src={logo} unoptimized width={240} /> : <span className="text-center text-lg font-bold text-white">{sponsor.name}</span>}</div><span className="mt-3 block text-sm font-bold text-white">{sponsor.name}</span>{sponsor.tier && <span className="mt-1 block text-xs text-acrux-muted">{sponsor.tier}</span>}</>;
