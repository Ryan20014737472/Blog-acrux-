    ...
    {members.map((member, index) => {
      ...
      return <ScrollReveal delay={index * 0.06} key={member.id}>
        <article className="glass-panel card-hover group h-full overflow-hidden rounded-2xl">
          {photo ? <img alt={`Retrato de ${member.name}`} className="h-42 w-full object-cover transition-transform duration-500 group-hover:scale-105" src={photo} /> : <PlaceholderMedia className="min-h-42 border-x-0 border-t-0" label={`Foto de ${member.name} pendente`} />}
