...
type Album = Pick<Database["public"]["Tables"]["galleries"]["Row"], "id" | "title" | "competition_id" | "is_published">;

interface CompetitionRelationsProps {
...
export function CompetitionRelations({ competitionId, canManage, disabled, onBusyChange }: CompetitionRelationsProps) {
    ...
    async function loadRelations() {
      try {
        ...
        const [memberResult, participantResult, albumResult] = await Promise.all([
          ...
          supabase.from("competition_team_members").select("team_member_id").eq("competition_id", competitionId),
          supabase.from("galleries").select("id, title, competition_id, is_published").order("title"),
        ]);
  ...
  }, [competitionId, onBusyChange, retry]);
  ...
  async function changeRelation(kind: "member" | "album", id: string, attach: boolean) {
    ...
    try {
      ...
      if (kind === "member") {
        const result = attach
          ? await supabase.from("competition_team_members").insert({ competition_id: competitionId, team_member_id: id }).select("team_member_id").single()
          : await supabase.from("competition_team_members").delete().eq("competition_id", competitionId).eq("team_member_id", id).select("team_member_id").single();
        ...
        setFeedback(attach ? "Participante vinculado à competição." : "Participante removido da competição. O perfil da equipe foi preservado.");
      } else {
        const query = supabase.from("galleries").update({ competition_id: attach ? competitionId : null }).eq("id", id);
        const { data, error: updateError } = await (attach ? query.is("competition_id", null) : query.eq("competition_id", competitionId))
          .select("id, title, competition_id, is_published").single();
        if (updateError || !data) throw new Error("Album update failed");
        if (version !== generation.current) return;
        setAlbums((current) => current.map((album) => album.id === data.id ? data : album));
        setAlbumId("");
        setFeedback(attach ? "Álbum vinculado à competição." : "Vínculo removido. O álbum e suas fotos foram preservados.");
      }
    } catch {
  ...
  const assignedAlbums = albums.filter((album) => album.competition_id === competitionId);
  const availableAlbums = albums.filter((album) => album.competition_id === null);
